  ...
  const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNncm9rZnptb2lpbG11cXlzY3lmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcyMDI1NDMsImV4cCI6MjA5Mjc3ODU0M30.L3vqMjSKWJ1K3WAdOLLPpAqxJ1uT9Ad4_49W4DaQeVw';
  ...
  async function loadMenu() {
    try {
      ...
      document.getElementById('order-items-list').innerHTML = '<div class="empty-state">Failed to load menu. Check your connection.</div>';
      document.getElementById('menu-grid').innerHTML = '<div class="empty-state">Failed to load menu.</div>';
    }
  }

  function renderMenu() {
    const grid = document.getElementById('menu-grid');
    if (!menu.length) { grid.innerHTML = '<div class="empty-state">No menu items yet</div>'; return; }
    grid.innerHTML = menu.map(item => `
      <div class="menu-item">
        ...
        <div class="price-row">
          ...
          <input type="number" id="price-${item.id}" value="${parseFloat(item.price).toFixed(2)}" min="0" step="0.01" />
  ...
  async function savePrice(id) {
    ...
    try {
      ...
      renderOrderItems();
    } catch (e) { showToast('Failed to save price'); }
  }
  ...
  async function addMenuItem() {
    ...
    if (!name || isNaN(price) || price < 0) { err.textContent = 'Please enter a valid name and price'; return; }
    err.textContent = '';
    try {
    ...
    } catch (e) { err.textContent = 'Failed to add item: ' + e.message; }
  }

  function renderOrderItems() {
    const list = document.getElementById('order-items-list');
    if (!menu.length) { list.innerHTML = '<div class="empty-state">No menu items</div>'; return; }
    list.innerHTML = menu.map(item => {
      ...
      return `<div class="order-item-row">
        ...
        <span class="item-subtotal">₾${(parseFloat(item.price) * qty).toFixed(2)}</span>
      </div>`;
    }).join('');
    updateSummary();
  }
  ...
  function updateSummary() {
    ...
    if (!items.length) {
      lines.innerHTML = '<div style="font-size:13px;color:#aaa;">No items added yet</div>';
  ...
  async function placeOrder() {
    ...
    const orderItems = items.map(i => ({ name: i.name, qty: cart[i.id], price: parseFloat(i.price) }));
    const btn = document.getElementById('place-btn');
    btn.disabled = true;
    btn.textContent = 'Placing...';
    try {
      await api('orders', 'POST', { items: orderItems, total, note: note || null });
      cart = {};
  ...
  async function loadAndRenderDaily() {
    ...
    try {
      ...
      const orders = await api('orders?created_at=gte.' + today + 'T00:00:00&order=created_at.desc');
      ...
      list.innerHTML = orders.map(o => {
        ...
        return `<div class="order-card">
          <div class="order-card-header">
            ...
            <span class="order-badge">₾${parseFloat(o.total).toFixed(2)}</span>
  ...
  async function deleteOrder(id) {
    ...
    try {
      ...
      loadAndRenderDaily();
    } catch (e) { showToast('Failed to delete order'); }
  }
  ...
  async function clearDay() {
    ...
    try {
      ...
      loadAndRenderDaily();
    } catch (e) { showToast('Failed to clear day'); }
  }
